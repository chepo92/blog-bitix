...
for f in static/assets/images/logotypes/*.svg; do inkscape --without-gui -w 750 "$f" --export-background white --export-background-opacity 1 --export-type png --export-filename "static/assets/images/structured-data/$(basename ${f%.*})-750.png"; done;
